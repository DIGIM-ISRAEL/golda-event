...
export default function LeadForm({ lead, flavors, locations, role, basketaCostNis }: LeadFormProps) {
  ...
  // Pricing
  const [extras, setExtras] = useState<Extra[]>(lead?.quote?.extras ?? [])
  ...
  function addExtra() {
    setExtras([...extras, { label: '', amount: 0 }])
  }
  ...
  async function addLocation() {
    ...
    const { data, error } = await supabase
      .from('locations')
      .insert({ city_name: newCity.trim(), travel_cost_nis: Number(newCityCost) })
      .select()
      .single()
    if (!error && data) {
  ...
  async function handleSave(status: string) {
    ...
    const leadData = {
      ...
      status,
      notes,
      manager_included: managerIncluded,
      assistants_count: assistantsCount,
      price_override: priceOverride ? Number(priceOverride) : null,
      price_per_person_override: pricePerPersonOverride ? Number(pricePerPersonOverride) : null,
    }

    let leadId = lead?.id

    if (isEdit) {
      const { error: e } = await supabase.from('leads').update(leadData).eq('id', lead!.id)
      if (e) { setError('שגיאה בשמירה'); setSaving(false); return }
    } else {
      const { data, error: e } = await supabase.from('leads').insert(leadData).select().single()
      if (e || !data) { setError('שגיאה ביצירת ליד'); setSaving(false); return }
      leadId = data.id
    }

    // Save flavors
    await supabase.from('lead_flavors').delete().eq('lead_id', leadId!)
    if (selectedFlavors.length > 0) {
      await supabase.from('lead_flavors').insert(
        selectedFlavors.map((fid) => ({ lead_id: leadId!, flavor_id: fid })),
      )
    }
    ...
    const quoteData = {
      lead_id: leadId!,
      base_price: pricing.basePrice + pricing.extraParticipantsPrice,
    ...
    if (lead?.quote?.id) {
      await supabase.from('quotes').update(quoteData).eq('id', lead.quote.id)
    } else {
          ...
          >
            ...
            {locationsList.map((l) => (
              <option key={l.id} value={l.id}>
                {l.city_name} — {formatNIS(l.travel_cost_nis)}
              </option>
            ))}
          </select>
          {/* Add new location */}
      ...
      <section className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-sm font-medium text-gray-600">
            {selectedFlavors.length}/{MAX_FLAVORS} נבחרו
          </span>
        </div>
      ...
      <section className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <button type="button" onClick={addExtra} className="text-sm text-blue-600 hover:text-blue-700">
              + הוסף תוספת
            </button>
          </div>
          {extras.map((extra, i) => (
            <div key={i} className="flex gap-2 mb-2">
              ...
              <input
                type="number"
                value={extra.amount}
                onChange={(e) => updateExtra(i, 'amount', Number(e.target.value))}
                placeholder="₪"
                className="w-24 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none"
              />
              <button
                type="button"
        ...
        <div className="bg-gray-50 rounded-xl p-4 space-y-2 text-sm">
          <div className="flex justify-between">
            ...
            <span>{formatNIS(pricing.basePrice)}</span>
          </div>
          {pricing.extraParticipantsPrice > 0 && (
            <div className="flex justify-between">
              <span className="text-gray-600">תוספת משתתפים</span>
              <span>{formatNIS(pricing.extraParticipantsPrice)}</span>
            </div>
          )}
          {pricing.logisticsCost > 0 && (
            <div className="flex justify-between">
              <span className="text-gray-600">לוגיסטיקה</span>
              <span>{formatNIS(pricing.logisticsCost)}</span>
            </div>
          )}
          {pricing.extrasTotal > 0 && (
            <div className="flex justify-between">
              <span className="text-gray-600">תוספות</span>
              <span>{formatNIS(pricing.extrasTotal)}</span>
            </div>
          )}
          {pricing.discountAmount > 0 && (
            <div className="flex justify-between text-red-600">
              <span>הנחה</span>
              <span>−{formatNIS(pricing.discountAmount)}</span>
            </div>
          )}
        ...
        <section className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="flex items-center gap-6">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                checked={managerIncluded}
                onChange={(e) => setManagerIncluded(e.target.checked)}
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                onChange={(e) => setAssistantsCount(Number(e.target.value))}
